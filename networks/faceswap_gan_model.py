...
class FaceswapGANModel():
    ...
    def build_train_functions(self, loss_weights=None, **loss_config):
        ...
        # Adam(...).get_updates(...)
        training_updates = Adam(lr=self.lrD*loss_config['lr_factor'], beta_1=0.5).get_updates(params=weightsDA, loss=loss_DA)
        self.netDA_train = K.function([self.distorted_A, self.real_A],[loss_DA], training_updates)
        training_updates = Adam(lr=self.lrG*loss_config['lr_factor'], beta_1=0.5).get_updates(params=weightsGA, loss=loss_GA)
        self.netGA_train = K.function([self.distorted_A, self.real_A, self.mask_eyes_A], 
                                      [loss_GA, loss_adv_GA, loss_recon_GA, loss_edge_GA, loss_pl_GA], 
                                      training_updates)

        training_updates = Adam(lr=self.lrD*loss_config['lr_factor'], beta_1=0.5).get_updates(params=weightsDB, loss=loss_DB)
        self.netDB_train = K.function([self.distorted_B, self.real_B],[loss_DB], training_updates)
        training_updates = Adam(lr=self.lrG*loss_config['lr_factor'], beta_1=0.5).get_updates(params=weightsGB, loss=loss_GB)
